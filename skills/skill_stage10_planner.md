...
A scene is affected if the specific character has lines, decisions, or behavior that contradict the arc being corrected — or if it's a key relationship scene between the characters named in the task. A scene where the character appears silently in the background is not affected. When CHARACTER data is provided in the prompt, use `arc.direction` and `arc.core_drive` only for Tier 1 arc-bearing characters. For Tier 2, evaluate whether the scene contradicts `functional_profile.narrative_function`, `emotional_truth`, `comic_or_tension_function`, `pressure_behavior`, or `voice_flavor`. For Tier 3, evaluate only scene purpose, casting energy, playable behavior, and any explicit line-style example.

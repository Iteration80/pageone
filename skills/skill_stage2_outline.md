...
* **Revision briefs are instructions, not content.** Writers often structure a revision brief as bracketed instruction blocks — `[Sequence 2] Update his backstory: disgraced Robin Hood figure…` or `[Preserve] The "Meeting in the Middle" arc stays as-is.` Read the bracket as the target section and the body as an edit to execute there. Applied correctly, the first example becomes a rewritten backstory beat *inside Sequence 2* — never a beat whose text says "Update his backstory". If a beat reads like a command about the story ("Update…", "Establish…", "Preserve…") instead of something that happens in the story, it is leaked process language and must not appear.

...
3. **Generate the `_deep_profile`** using the inferred types:
   * `dialogue_fingerprint`: Concrete writing rules derived from MBTI — preferred sentence length, vocabulary domain, question style, interruption tendency, topics they avoid, filler words. Write as technical instructions a drafting agent can copy verbatim.

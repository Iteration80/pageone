...
async function persistKnowledgeSourceAssets({
    ...
    const chunkIndex = {
        ...
        chunkOverlap: SOURCE_CHUNK_OVERLAP,
        chunks: buildSourceChunks(fileText, Number.POSITIVE_INFINITY)
    };
    const chunkIndexPath = path.join(sourceDir, 'chunks.json');
    await fs.writeFile(chunkIndexPath, JSON.stringify(chunkIndex, null, 2), 'utf8');
    ...
    return {
        ...
        chunkIndex: {
            ...
            path: normalizeStoredRelativePath(chunkIndexPath),
            chunkCount: chunkIndex.chunks.length,
            generatedAt: chunkIndex.generatedAt
        }
    };
}
...
function buildSourceChunks(text, maxChunks = SOURCE_CHUNK_LIMIT) {
    const chunks = [];
    let index = 0;
    let offset = 0;
...
async function persistChatAttachmentToKnowledge(projectData, attachment, { stageId, userMessage, originTag = 'chat_upload', projectId = null } = {}) {
    ...
    if (existing) {
        ...
        if (sourceAssetProjectId && (!existing.originalFile || !existing.extractedMarkdown)) {
            const assets = await persistKnowledgeSourceAssets({
            ...
            if (assets) {
                ...
                existing.chunkIndex = existing.chunkIndex || assets.chunkIndex;
            }
        }
        return {
            fileText,
            savedSource: {
                ...
                chunkIndex: existing.chunkIndex || null
            }
        };
    }
    ...
    if (assets) {
        ...
        entry.chunkIndex = assets.chunkIndex;
    }
...
function resolveStoredDataPath(relativePath) {
    ...
    if (!fullPath.startsWith(DATA_ROOT + path.sep) && fullPath !== DATA_ROOT) return null;
    return fullPath;
}

function loadSourceChunkIndex(source) {
    const indexPath = resolveStoredDataPath(source?.chunkIndex?.path);
    if (!indexPath) return [];
    try {
        const parsed = JSON.parse(fsSync.readFileSync(indexPath, 'utf8'));
...
function sourceSegments(source) {
    const indexedChunks = loadSourceChunkIndex(source);
    if (indexedChunks.length) {
        return indexedChunks.map(chunk => ({ label: `${source.name} / chunk ${chunk.index + 1}`, text: chunk.text, source }));
    }
...
async function initDb() {
    try {
        ...
        await fs.mkdir(SOURCE_FILES_DIR, { recursive: true });

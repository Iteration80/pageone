...
- If the writer later answers one decision from your triage, keep the next response scoped to that decision or the same recommended batch. Do not pivot to unrelated lower-priority craft notes.
...
4. One recommended first batch, with scene numbers if clear.\n\n`;
            ...
            if (userExchangeCount >= 5) {
                conversationPrompt += `\n\n## DECISION CHECKPOINT (MANDATORY)\nThis is exchange ${userExchangeCount}. You MUST now stop the conversation from sprawling. Do not ask another broad clarifying question unless the writer explicitly asked to keep brainstorming.\n\n1. Name the strongest current recommendation or the active unresolved fork in one short paragraph or 2 bullets. Do NOT recap the whole conversation unless the writer seems lost or the direction genuinely changed.\n2. Choose the appropriate next step based on what ACTUALLY happened:\n   - If YOU surfaced a specific improvement or issue in THIS response: ask the writer whether they want to address it before moving on. Follow through on your own observations — do NOT pivot to a generic "anything else?" when you just identified something concrete.\n   - If concrete changes were proposed or agreed on: offer to apply the scoped change or scoped batch ("Want me to go ahead and update the [stage output] with that?"). Set suggest_plan: true.\n   - If the writer just resolved one decision from a triage: confirm that decision and keep the next move inside the same active batch or the next unresolved decision from that same triage. Do NOT introduce dormant notes from the wider feedback as competing alternatives.\n   - If the conversation was purely exploratory and no improvements were surfaced: recommend the next useful angle or say the artifact looks ready to approve. Do NOT offer to regenerate — there is nothing to regenerate. Set suggest_plan: false.\n\n`;
            } else if (userExchangeCount >= 4) {
                conversationPrompt += `\n\n## DECISION CHECKPOINT REMINDER\nThis is exchange ${userExchangeCount}. On your next response, avoid another open-ended question unless the writer asks to keep exploring. Be ready to name the strongest current recommendation or active unresolved fork. Only offer to regenerate/update content if concrete changes were actually discussed, and do not revive dormant notes from earlier feedback as competing next steps.\n\n`;
            }
            if (Number(stageId) === 4) {
    ...
    try {
        ...
        if (isInit) {
            ...
            if (userExchangeCount >= 7) {
                conversationPrompt += `\n\n## DECISION CHECKPOINT (MANDATORY)\nThis is exchange ${userExchangeCount}. You MUST now stop the rewrite discussion from sprawling. Do not ask another broad clarifying question unless the writer explicitly asked to keep brainstorming.\n\nName the strongest current rewrite recommendation or unresolved fork in one short paragraph or 2 bullets. Do NOT recap the whole conversation unless the writer seems lost or the direction genuinely changed. If there is enough direction to plan a concrete rewrite, offer to proceed with a rewrite plan and set suggest_plan: true.\n\n`;
            } else if (userExchangeCount >= 6) {
                conversationPrompt += `\n\n## DECISION CHECKPOINT REMINDER\nThis is exchange ${userExchangeCount}. On your next response, avoid another open-ended question unless the writer asks to keep exploring. Be ready to name the strongest current rewrite recommendation or unresolved fork. Only offer a rewrite plan when the direction is concrete enough to execute.\n\n`;
            }

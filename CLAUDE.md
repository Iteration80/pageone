...
1. Google sign-in is active only when `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, and `ALLOWED_EMAILS` are set. The server issues a signed `pageone_session` cookie after a successful Google OAuth callback, and the allowlist is rechecked on every request. **Since multi-user Phase 4 the allowlist is `ALLOWED_EMAILS` ∪ a data-backed list** (`<DATA_ROOT>/access-control.json`, edited from Settings → Administration by an admin) — the env is the floor and additive-only: an env address cannot be removed from the UI, a store address removed from the UI loses cookie AND token access on its next request. **Admins** are `ADMIN_EMAILS` (optional env) ∪ the store's admins; if neither names anyone, the first `ALLOWED_EMAILS` address is the *bootstrap* admin (that was the whole rule from Phase 2 to Phase 4 — now the fallback of last resort; it switches off the moment anyone is named, and promoting persists the acting admin so the first promotion cannot lock them out). `utils/auth.js` `isAllowedEmail` / `isAdminEmail` delegate to `utils/access_control.js`; nothing else decides.
...
⚠️ **A token cannot manage tokens.** `/api/tokens` is guarded by a session-only check inside `routes/tokens.js`, not by `requireAuth` — otherwise one leaked token could mint its own successors and revoke the real ones, surviving revocation of the credential that leaked. **The same rule guards `/api/admin/*` mutations** (allowlist, admins, quotas — `routes/admin.js` `requireAdminSession`): a leaked admin token must not be able to add its holder's own address and outlive its own revocation. Only `GET /api/admin/overview` accepts an admin token, so a script can report spend.

**Per-user monthly quotas (Phase 4).** USD per calendar month (UTC): `defaultMonthlyUsd` applies to non-admins, `perUser[email]` overrides for anyone (`null` = explicitly unlimited). Enforced by `quotaGuard` in server.js, **composed into `aiLimiter`/`strictLimiter`** (`withQuota`) so every rate-limited AI route is budget-checked with no second route list; a soft cap on *starting* work (a stream that crosses the line completes; the next request is 429 `QUOTA_EXCEEDED`). Month-to-date spend is priced server-side with **the ONE price table, `public/model-pricing.js`** — shared with the browser like `screenplay-layout.js`; never add a rate anywhere else — and cached per owner (rebuilt ≤ once a minute, bumped in place by `trackUsage`). Break-glass and open mode carry no email and are never capped.
...
### 2026-08-16 — Stage 10 "Script": the finished screenplay as a read-only view (internal id 11)
Visible stage 10, unlocked (and marked complete) by `stage9_rewrites.approved` — the same signal, no data key of its own (`STAGE_DATA_KEYS` deliberately has no `11:` entry). Scene text resolves **exactly as the exporters resolve it**: the rewrite's `working` entries sorted by number with `[SCENE DELETED]` dropped, else the approved draft (`humanized_draft_text || draft_text`) — so the page shows what `/api/export/pdf` prints by construction (browser-verified: view said 117 pages, the exported PDF is physically 118 = 117 + the uncounted title page). Page chips + boundary markers reuse the Stage 7 overlay's walk over `window.ScreenplayLayout` (never a second estimate). Header buttons: `.fountain` (client-side), `.docx`, `Export PDF` (starred for a rewrite), `PDF (clean)` = `?marks=0`, shown only when a rewrite exists (a first draft has nothing to star); all reuse the existing export endpoints with `stage=rewrite|draft` picked by what the view shows. 7 structural pins in `test/stage11_script_view.test.js` (475 total); the knowledge suite's "nine visible stages" pin updated to ten.

### 2026-08-16 — `npm run backup:prod` / `npm run push:prod` (PAT-authenticated pull-backup + explicit per-project push)
The 08-09 decision made concrete: **no continuous two-way sync**, two explicit verbs. `scripts/lib/prod_sync.js` holds the logic (testable against the harness); `scripts/backup-prod.js` and `scripts/push-prod.js` are thin CLIs. Config via env or the repo's `.env`: `PAGEONE_URL` (default prod), `PAGEONE_TOKEN` (a `pgo_…` PAT from Settings → Access Tokens), `PAGEONE_BACKUP_DIR` (default `backups/prod`, gitignored). Nothing lives outside COWORK → no `machine-dependencies.md` row.
**backup:prod** mirrors every project the token can see to `backups/prod/latest/projects/<id>.json`, writes `manifest.json` with each project's **content hash at pull time**, and a dated copy under `snapshots/<iso>/` (`--no-snapshot` to skip). `latest/` is a mirror — a project gone from prod is dropped there and survives in earlier snapshots. **An admin token pulls everyone's** through two new admin routes, `GET /api/maintenance/projects` (id/title/owner/bytes/sha256) and `GET /api/maintenance/projects/:id` — ⚠️ **the one place a scoped request is deliberately elevated to a system read** (`runAsSystem` = `runWithIdentity(null, …)`, inside `requireAdmin`, read-only, logged; it reuses the chokepoint rather than reading the directory, so it is not a fifth direct reader). `--mine` narrows an admin to their own.
**push:prod -- <id>** uploads ONE project from `latest/` (or `--file`) via `PUT /api/projects/:id`, **refusing** (exit 2) when prod's hash ≠ the manifest's (`CHANGED_ON_REMOTE`), when the id was never pulled (`NEVER_PULLED`), when the local file's `id` is another project (`ID_MISMATCH`), or when prod has no such project for this token (`NOT_ON_REMOTE` — push never creates, and cannot reach another owner's project). `--force` overrides the first two only. Identical → no-op. After a push it re-reads prod and updates `latest/` + manifest. ⚠️ The server's PUT merges `data` shallowly, so a key deleted locally survives on prod — push restores/overwrites, it does not prune (documented in `--help`).
**Hardening found on the way:** `PUT /api/projects/:id` spread the whole body over the record, so `{owner:"bob@…"}` would have dropped a project into Bob's list and `{owner:""}` would have made it fail closed. `id` and `owner` are now server-pinned in the PUT (like a style's provenance re-stamp). 5 tests in `test/prod_sync.test.js` (468 total); guard breaks: changed-on-remote off → 1 · never-pulled off → 1 · owner pin off → 1 · maintenance export unguarded → 1 · NOT_ON_REMOTE off → 1. Both CLIs smoke-tested against a local open-mode server on a copy of the real store (pull 10 → unchanged → refuse on drift → force).

### 2026-08-16 — Multi-user Phase 4: admin surface (allowlist, admins, quotas) + one price table
**Access control is data-backed and env-additive.** `utils/access_control.js` keeps `<DATA_ROOT>/access-control.json` (`allowed`, `admins`, `quotas`) with mtime-cached synchronous reads (so `isAllowedEmail`/`getSessionEmail` stay sync and per-request live) and serialised atomic writes (tokens.js contract: no-op updaters return `false`, no write). `isAllowedEmail` = `ALLOWED_EMAILS` ∪ store; `isAdminEmail` = `ADMIN_EMAILS` ∪ store, bootstrap = first `ALLOWED_EMAILS` address only when neither names anyone. **The provisional "first address is admin" rule is retired to a fallback** — prod needs no config change (Carsten is first), and the startup log says which admins are effective and from where. Removing an address from the store also drops it from the store's admins (no phantom admins). Promoting persists the acting admin if their standing was bootstrap-only.
**Routes** (`routes/admin.js`): `GET /api/admin/overview` (requireAuth+requireAdmin — allowlist with sources, admins with sources, quotas, and per-owner usage priced month-to-date + all-time, every allowlisted person listed even at $0) · `POST/DELETE /api/admin/allowlist[/:email]` · `POST/DELETE /api/admin/admins[/:email]` · `PUT /api/admin/quotas` — **mutations are session-only** (a token cannot edit who may enter). Refusals: env-listed → 409, self-remove/self-demote → 400, promote-not-allowlisted → 400, bootstrap-demote → 409 (promote someone explicitly instead). `/api/me` now returns `admin`. `/api/maintenance/*` honours the store via the same `isAdminEmail`.
**Quotas** — `quotaGuard` composed into both AI limiters via `withQuota` (see Deployment Notes); 429 `QUOTA_EXCEEDED` with "$spent of $cap"; `usageRollup` gained `since`; `trackUsage` bumps the per-owner cache. **Pricing** moved from app.js to `public/model-pricing.js` (browser+server), added `gemini-3.6-flash` ($0.75/$3.75 through 2026-12-31 per Google's live page — it was pricing at $0.00), and corrected two stale Gemini rows against the same page (3 Flash $0.10/$0.40 → $0.50/$3.00; 3.1 Pro $1.25/$10 → $2/$12; Claude rows verified unchanged). `priceUsage()` reports unpriced models so the modal shows a † instead of a silent zero. ⚠️ Wrapped in an IIFE — a top-level `const MODEL_PRICING` in a classic script clashed with app.js's alias and killed app.js at load.
**UI** — Settings → Administration (admin only; `/api/me.admin`): default monthly budget, one two-line row per person (chips: admin/env/bootstrap/added-by · projects · this month/all time with per-model tooltip · budget input: empty = default, number = cap, "none" = unlimited · Make admin/Demote · Remove, disabled with the reason for env/self), read-only rows for owners no longer allowlisted, add-to-allowlist. Browser-verified end to end under two identities on a copy of the real store.
**Tests** — 10 in `test/admin_access_control.test.js` (463 total). Break-the-guard signatures: store allowlist ignored → 2 · token may mutate → 2 · quota guard off → 2 · admin default exemption off → 1 · month window off → 1 · actor-persist on promote off → 1 · env floor removable → 1 · bootstrap removed → 8. Completeness walk of `/api/admin/` in the live route table. Two `/api/me` shape assertions updated for the new `admin` field.
...
### 2026-08-16 — Style sharing: Share / Make private + Copy to my library
Carsten's design, same day: sharing is **copy-based**. `visibility: private|shared` in the front matter (server-owned; default private, no migration). `callerMayAccess` admits another writer's shared style for viewing; `callerMayUse` (attach to a project, draft with) and `callerMayModify` stay owner-only — `select-style`, `preview-style-scene` and `loadProjectStyle` refuse a shared-not-mine style (403 with "copy it to your library"). `POST /api/styles/:slug/copy` mints a private copy the caller owns (both files, `copied_from`, `paired_with` rewritten, source `project_id` dropped) — works on shared styles and on presets. UI: owner sees **Share** / **Make private** in the detail modal (trained styles get a note that the reference is shared too); others see "Shared by <email> — read-only" + **Copy to my library**; the Stage 6 picker shows **Copy & Use** for shared styles (copies, then selects the copy; `stage7MarkActiveCard` restores each card's own idle label instead of assuming "Use This"). A refine-in-place carries `visibility` over. 3 more route tests (20 in the file); guard breaks: use-check off → 2, modify back to access-based → 1, copy keeps source owner → 1. ⚠️ `callerMayModify` is defined on `callerMayUse`, not `callerMayAccess` — with sharing, "may see" is wider than "may edit".
...
**Two cross-tenant leaks found in the route audit and closed:** the `/api/maintenance/*` family swept every tenant's projects (two of them writing) behind plain `requireAuth`, and the global style creator fed **every** project's title, genre and logline into the prompt as "WRITER'S PROJECTS". Maintenance is now `requireAdmin`; the style context is owner-scoped. ⚠️ `isAdminEmail` (utils/auth.js) was **provisional** — the first address in `ALLOWED_EMAILS`. Phase 4 (2026-08-16, above) replaced it with the data-backed admin list; the first-address rule survives only as the bootstrap fallback.
